...
sharing <- sharing %>%  as.data.frame() %>%
  mutate(shared.org = as.numeric(shared)/as.numeric(org2.count))  %>% # create sharing index for org1
  select(- shared.org1, -shared.org2)
save(sharing, file = "data/sharing/species.gen.sharing.rda")
```
...
save(spec_predict, mylm, species_all, file = "data/phylo/joined.species.rda")
p1 <- ggplot(spec_predict, aes(x = V1, y = predicted))+
  geom_point()+
