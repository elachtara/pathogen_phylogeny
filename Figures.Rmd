...
p1 <- species_species %>% 
  ...
  geom_point(col = mypal[4], size = 1)+
  geom_smooth(method = "lm", se = FALSE, size = 0.3, col = mypal[1])+
  theme_minimal()+
...
p1a <- genus_species_all %>%
  ggplot(aes(x = shared.org))+
  geom_density(fill = mypal[7])+
  theme_minimal()+
  labs(y = "Count", x = "Jaccard Index")+
