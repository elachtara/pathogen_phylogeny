...
p <- genus_all.p %>% 
  ...
  geom_point(col = mypal[4])+
  geom_smooth(method = "lm", se = FALSE, color = mypal[1])+
  theme_minimal()+
  labs(y = "Proportion Pathogens Shared", x = "Phylogenetic Distance")+
  labs(title = "Pathogen sharing vs. Distance (Genus-Genus)")+
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
...
p2 <- genus_all.p %>% 
  filter(shared.org > 0) %>%
  ggplot(aes(x = distance, y = log(shared.org))) +
  geom_point(col = mypal[5])+
  geom_smooth(method = "lm", se = FALSE, color = mypal[1])+
  theme_minimal()+
  labs(y = "log(Proportion Pathogens Shared)", x = "Phylogenetic Distance")+
  labs(title = "Pathogen sharing vs. Distance (Genus-Genus)")+
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        axis.title.y = element_text(size = 12, face = "bold"),
        axis.title.x = element_text(size = 12, face = "bold"),
...
ggsave("figures/DOT/genus.log.pdf",
       plot = p2,
       units = "in",
       width = 6,
       height = 6,
